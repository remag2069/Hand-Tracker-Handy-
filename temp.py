...
def gen_mask(frame):
    ...
    global_mask = cv2.dilate(global_mask,np.ones((3,3), np.uint8),iterations = 2)
...
def gen_contour(thresh,frame):
    ...
    if l==0:
        ...
        cv2.circle(drawing_max, (int(sum_x/l),int(sum_y/l)), 3, [0,0,255], -1)
...
while(1):
    ...
    x,y=gen_contour(global_mask,frame)
